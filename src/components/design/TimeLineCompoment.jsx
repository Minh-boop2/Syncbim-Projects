	...
	return (
		<section className="relative mx-auto max-w-7xl px-4 py-16">
			...
			<button
				...
				onMouseEnter={() => setIsHovered(true)}
				onMouseLeave={() => setIsHovered(false)}
				className={`absolute right-4 top-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full transition-all duration-300 ${canScrollRight ? (isHovered ? "opacity-100" : "opacity-0") : "pointer-events-none opacity-0"} bg-gradient-to-br from-blue-500 to-cyan-500 text-white shadow-md shadow-blue-500/30 hover:scale-110 hover:shadow-lg hover:shadow-blue-500/50 dark:from-gray-700 dark:to-gray-900 dark:shadow-gray-700/30 dark:hover:shadow-gray-600/50`}
			>
				→
			</button>
			{/* Line ngang: chỉ md+ */}
			...
			<div
				ref={timelineRef}
				className="relative flex flex-col gap-8 overflow-x-auto md:w-full md:-translate-x-3 md:flex-row md:gap-12 md:overflow-hidden md:pb-6 md:pt-6"
				onMouseEnter={() => setIsHovered(true)}
				onMouseLeave={() => setIsHovered(false)}
			>
				{items.map((item, i) => (
			...
			>
				{isTop ? (
					<div className="mb-10 flex w-[220px] flex-col items-center text-center">
						<h3 className="text-3xl font-bold shadow-xl" style={{ color }}>
							{item.year}
						</h3>
						<div
				...
				{!isTop ? (
					<div className="mt-10 flex w-[220px] flex-col items-center text-center">
						<h3 className="text-3xl font-bold shadow-2xl" style={{ color }}>
							{item.year}
						</h3>
						<div
							className="mt-2 h-8 w-0.5"
...
function Bullets({ bullets, color, wide = false }) {
	return (
		<div
			className={`mt-2 rounded-lg border border-gray-200/70 bg-white/80 p-3 text-sm text-gray-700 shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-n-8 dark:text-gray-200 ${
